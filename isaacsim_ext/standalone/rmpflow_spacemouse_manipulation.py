...
from isaacsim_ext.controller import RMPFlowController
from isaacsim_ext.ros_bridge import PoseSubscriber
...
action_subscriber = PoseSubscriber(
    robot_arm=ur5e,
    target=target,
    gripper=gripper,
)
...
while simulation_app.is_running():
    ...
    if my_world.is_playing():
        ...
        if action.gripper_action is not None:
            gripper.apply_action(action.gripper_action)
